...
@interface KKEmoticonTool()<KKImageEditToolItemDelegate>
{
...
- (void)setEmoticonMenu
{
    ...
    for(NSString *path in list){
        ...
        if(image){
            ...
            KKImageEditToolItem *item = [[KKImageEditToolItem alloc]initWithFrame:CGRectMake(x + padding, 0, W, H)];
            item.delegate = self ;
...
- (void)setPopupMenuView
{
    ...
    for(NSString *path in list){
        ...
        KKImageEditToolItem *item = [[KKImageEditToolItem alloc]initWithFrame:CGRectMake(x + wPadding, y, W, H)];
        item.delegate = self ;
        item.itemImage = [image aspectFit:CGSizeMake(45, 45)];
        item.editInfo = @{@"filePath" : filePath} ;
        item.hideTitle = YES ;
...
- (void)imageEditItem:(KKImageEditToolItem *)item clickItemWithType:(KKImageEditType)editType editInfo:(NSDictionary *)editInfo
{
    ...
    if(![filePath isEqualToString:@"showMore"]){
        ...
        KKEmoticonView *view = [[KKEmoticonView alloc] initWithImage:[UIImage imageWithContentsOfFile:filePath]];
...
- (void)showModalViewWithView:(UIView *)view
{
    ...
    _modalPopupView = [[KKModalView alloc]init];
